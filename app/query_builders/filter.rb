...
class Filter
  ...
  def cont(column, value)
    @scope.where(ActiveRecord::Base::sanitize_sql("#{column} LIKE '%#{value}%'"))
  end

  def notcont(column, value)
    @scope.where(ActiveRecord::Base::sanitize_sql("#{column} NOT LIKE '%#{value}%'"))
  end
  ...
  def gt(column, value)
    @scope.where(ActiveRecord::Base::sanitize_sql("#{column} > '#{value}'"))
  end
